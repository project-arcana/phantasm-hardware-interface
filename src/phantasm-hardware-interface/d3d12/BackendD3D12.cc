...
phi::init_status phi::d3d12::BackendD3D12::initialize(const phi::backend_config& config)
{
    ...
    // Global pools - except for swapchain pool
    {
        ...
        mPoolResources.initialize(device, &mAdapter.getAdapter(), config.max_num_resources, config.max_num_swapchains, config.static_allocator,
                                  config.dynamic_allocator);
