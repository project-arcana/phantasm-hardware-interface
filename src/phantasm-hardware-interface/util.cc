...
void phi::util::shader_table_offsets::init(const phi::shader_table_sizes& record_sizes,
                                           uint32_t num_ray_gen_stacks,
                                           uint32_t num_miss_stacks,
                                           uint32_t num_hit_group_stacks,
                                           uint32_t num_callable_stacks)
{
    this->record_sizes = record_sizes;
    ...
    auto const ray_gen_full_stack_size = num_ray_gen_stacks * align_up(record_sizes.size_ray_gen, 64);

    offset_miss_base = ray_gen_full_stack_size;
    auto const miss_full_stack_size = num_miss_stacks * align_up(record_sizes.size_miss, 64);

    offset_hit_group_base = offset_miss_base + miss_full_stack_size;
    auto const hit_group_full_stack_size = num_hit_group_stacks * align_up(record_sizes.size_hit_group, 64);

    offset_callable_base = offset_hit_group_base + hit_group_full_stack_size;
    auto const callable_full_stack_size = num_callable_stacks * align_up(record_sizes.size_callable, 64);
